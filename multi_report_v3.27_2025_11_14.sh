...
#  DONE - 8. Include the "render_case" version number if Mapper_Drive="enable"
...
###### Disk Layout ######
Disklayout="disable"		# BETA TESTING ONLY and SCALE ONLY.  Options: "enable" or "disable".
MS_Outlook="enable"			# Set to "enable" to use the OUTLOOK compatible version.  Default="disable".
...
CurrentFilename="multi_report_v3.27_2025_11_14.txt"
valid_config_version_date="2025-11-04"	# Configuration file valid date
...
mapper_version=0.1							# Format Major.Minor

# Assign Variables
mapper_html_file="$SCRIPT_DIR/case_email_snippet.html"
mapper_json_file="$SCRIPT_DIR/disklayout_config.json"
tmpfile="/tmp/mapper.tmp"

# Define arrays
declare -a mapper_slot						# The Grid Slot number (probably replaces the Bay value)
declare -a mapper_name						# Drive ID
declare -a mapper_serial					# Drive Serial Number
declare -a mapper_model						# Drive Model
declare -a mapper_capacity					# Drive Capacity
declare -a mapper_uuid						# Drive UUID (if exist)
declare -a mapper_addr						# HCTL of the drive data connector
declare -a mapper_bay						# The drive bay/slot number assigned to the HCTL address
declare -a mapper_pool						# The drive is part of this pool
...
declare -a mapper_color						# The drive color will change to indicate a problem
declare -a mapper_temperature				# The current drive temperature
...
mapper_join_chart() {
if [[ $Debug_Steps == "true" ]]; then echo "mapper_join_chart" | tee -a /tmp/multi_report_debug.txt; fi
output=$logfile
page1="page1.html"
page3="page3.html"

if [[ $MS_Outlook == "enable" ]]; then
	page2="case_email_outlook_snippet.html"
else
	page2="Disk_layout.html"
fi

{
  ...
  echo "pre { color: #000; }"   # keep pre text readable without overriding other styling
  echo "</style>"
  echo "</head>"
  echo "<body>"
  cat "$page1"
  ...
  echo "<h2>Drive Layout Visualization</h2>"
	...
	if [[ ${mapper_slot[0]} == "X" || ${mapper_slot[0]} == "" ]]; then
  ...
  if [[ -f "$page3" ]]; then
    ...
    if [[ -n "$start_line" ]]; then
      # include the preceding line as well (to get the <br> which you showed)
      start=$((start_line - 1))
      if [[ $start -lt 1 ]]; then start=1; fi
      sed -n "${start},\$p" "$page3"
    else
      # marker not found — fallback to whole file
      cat "$page3"
    fi
  else
    echo "<!-- missing file: $page3 -->"
  fi
...
mapper_join_html() {
if [[ $Debug_Steps == "true" ]]; then echo "mapper_join_html"; fi
...
cat $mapper_html_file > Disk_layout.html
...
mapper_set_values() {
if [[ $Debug_Steps == "true" ]]; then echo "mapper_set_values"; fi
...
while [[ $array_index -le $array_size ]]; do
	...
	if [[ "${mapper_serial[array_index]}" == "$serial" ]]; then
		mapper_temperature[array_index]=$temp
		mapper_color[array_index]=$temp_color
		return
	fi
    (( array_index++ ))
done
...
mapper_collect_drive_data() {
if [[ $Debug_Steps == "true" ]]; then echo "mapper_collect_drive_data"; fi
# Initialize counters
lsscsi_line=1  # Tracks the current line number in the unfiltered lsscsi output
array_index=0  # Tracks the index for the arrays (only increments for valid drives)

# Get the total count of *all* devices from lsscsi (needed for the loop condition)
# We increment the count because the loop uses -le and starts at 1
total_lsscsi_count=$(lsscsi | wc -l)

#echo ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>   Collect the data    <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<"
#echo "OUTPUT FROM lsscsi:"
#echo "Name, Serial, Model, Capacity, UUID, Address, Drive Color"

# Loop through ALL lines of the lsscsi output
while [[ $lsscsi_line -le $total_lsscsi_count ]]; do
    
    # Get the device name (e.g., sda)
    MAPPER_DEVICE_NAME=$(lsscsi | awk '{print $NF}' | cut -d'/' -f3 | head -n ${lsscsi_line} | tail -n 1)
	MAPPER_DEVICE_DISK=$(lsscsi | awk '{print $2}' | head -n ${lsscsi_line} | tail -n 1)
	
    # Check if a device name was actually returned (handles the final empty line)
    if [[ -z "$MAPPER_DEVICE_NAME" ]] || [[ "$MAPPER_DEVICE_DISK" != "disk" ]]; then
#		echo "Bypass - Not a Disk" | tee -a debug.txt
        (( lsscsi_line++ ))
        continue
    fi

    # Get the drive model using lsblk on the current device
    # Use -n (no headers) and -d (disk only) for a clean, single line
    MAPPER_MODEL_NAME=$(lsblk -n -d -o MODEL "/dev/${MAPPER_DEVICE_NAME}")

    # The grep -q is quiet; the -v means "select non-matching lines"
    if echo "$MAPPER_MODEL_NAME" | grep -qvi "vmware\|virtual"; then
        # If the MAPPER_MODEL_NAME is does not contain VMWare or virtual, proceed to collect and store data

        # Save data in the array.
        mapper_name[array_index]="$MAPPER_DEVICE_NAME"
        mapper_model[array_index]="$MAPPER_MODEL_NAME"
        
        # Get drive serial number
        mapper_serial[array_index]=$(lsblk -n -d -o SERIAL "/dev/${MAPPER_DEVICE_NAME}")
		
        # Get the full lsscsi -s line for capacity and address
        MAPPER_CAP_ADDR_LINE=$(lsscsi -s | head -n ${lsscsi_line} | tail -n 1)

        # Get capacity: removes the device path and takes the last field (the size)
        mapper_capacity[array_index]=$(echo "$MAPPER_CAP_ADDR_LINE" | sed 's/\( \/dev\/[a-z]\+\)$//' | awk '{print $NF}')
		
        # Get drive UUID (PTUUID for the whole disk is generally better)
        mapper_uuid[array_index]=$(lsblk -n -o PARTUUID "/dev/${MAPPER_DEVICE_NAME}" | tail -n 1)
		
        # Get drive address
        mapper_addr[array_index]=$(echo "$MAPPER_CAP_ADDR_LINE" | cut -d'[' -f2 | cut -d']' -f1)
		
		# Get pool name
		if [[ ${mapper_pool[array_index]} == "" ]]; then
			mapper_pool[array_index]=$(blkid -s PARTUUID -s LABEL | grep -i ${mapper_uuid[array_index]} | cut -d'"' -f2)
		fi
		if [[ ${mapper_pool[array_index]} == ${mapper_uuid[array_index]} ]]; then
			mapper_pool[array_index]="Not Assigned"
		fi
  
        # Only increment the array index if data was collected
        (( array_index++ ))
    fi
	
    # Always increment the lsscsi line index to move to the next device in the list
    (( lsscsi_line++ ))
...
mapper_read_json_file() {
if [[ $Debug_Steps == "true" ]]; then echo "mapper_read_json_file"; fi
	...
	# Read the Version and Date of the json file.
	mapper_read_version=$(cat $mapper_json_file | jq -Mre '.version[].version | values')
	mapper_read_version_date=$(cat $mapper_json_file | jq -Mre '.version[].last_updated | values')

	# Initialize counters
	lsscsi_line=0  # Tracks the current line number in the unfiltered lsscsi output.
	total_lsscsi_count=$(grep -o "slot" $mapper_json_file | wc -l)

	# Loop through ALL entries of the json file
	while [[ $lsscsi_line -lt $total_lsscsi_count ]]; do
		# Read individual values and place in array.
		mapper_slot[lsscsi_line]=$(cat $mapper_json_file | jq -Mre '.bays['${lsscsi_line}'].slot | values')
		mapper_pool[lsscsi_line]=$(cat $mapper_json_file | jq -Mre '.bays['${lsscsi_line}'].pool | values')
		mapper_name[lsscsi_line]=$(cat $mapper_json_file | jq -Mre '.bays['${lsscsi_line}'].drive_id | values')
		mapper_serial[lsscsi_line]=$(cat $mapper_json_file | jq -Mre '.bays['${lsscsi_line}'].serial | values')
		mapper_model[lsscsi_line]=$(cat $mapper_json_file | jq -Mre '.bays['${lsscsi_line}'].model | values')
		mapper_capacity[lsscsi_line]=$(cat $mapper_json_file | jq -Mre '.bays['${lsscsi_line}'].capacity | values')
		mapper_uuid[lsscsi_line]=$(cat $mapper_json_file | jq -Mre '.bays['${lsscsi_line}'].uuid | values')
		mapper_color[lsscsi_line]=$(cat $mapper_json_file | jq -Mre '.bays['${lsscsi_line}'].drive_color | values')
		mapper_temperature[lsscsi_line]=$(cat $mapper_json_file | jq -Mre '.bays['${lsscsi_line}'].drive_temp | values')
		
		# Always increment the lsscsi line index to move to the next device in the list.
		(( lsscsi_line++ ))
	done
	# This should only be active on the second run of MR.
	if [[ ${mapper_slot[0]} == "" ]]; then mapper_slot[0]="X"; fi
}

mapper_write_json_file() {
if [[ $Debug_Steps == "true" ]]; then echo "mapper_write_json_file"; fi
    # Write the entire file, rebuilding version and bays sections.
	if [[ ${mapper_slot[0]} == "X" ]]; then
		echo
		echo
		echo "<><><><><><><><><><><><><><><><><><><><><><><><><><><><><><>"	
		echo "<>         YOUR CONFIG FILE IS NOT COMPLETE.              <>"
    ...
    mapper_name_count=${#mapper_name[@]} # Get the drive count
    mapper_read_version=$mapper_version
    mapper_read_version_update="$(date)"

    {
        echo '{'
        echo '  "version": ['
        echo '    {'
        echo "      \"version\": \"${mapper_read_version}\","
        echo "      \"last_updated\": \"${mapper_read_version_update}\""
        echo '    }'
        echo '  ],'
        echo '  "bays": ['

        first_entry=true
        for ((count=0; count<mapper_name_count; count++)); do
		    if [[ -z "${mapper_name[count]}" ]]; then
                continue
            fi
            ...
            cat <<EOF
    {
      "slot": "${mapper_slot[count]}",
      "address": "${mapper_addr[count]}",
      "pool": "${mapper_pool[count]}",
      "drive_id": "${mapper_name[count]}",
      "serial": "${mapper_serial[count]}",
      "model": "${mapper_model[count]}",
      "capacity": "${mapper_capacity[count]}",
      "uuid": "${mapper_uuid[count]}",
      "drive_color": "${mapper_color[count]}",
      "drive_temp": "${mapper_temperature[count]}"
    }
EOF
        done
        echo
   ...
   if test -e "$mapper_json_file"; then

        # Merge the new version/bays with the existing "case" section
        if jq --slurp '.[0] * { case: .[1].case }' "$tmpfile" "$mapper_json_file" > "${tmpfile}.merged"; then
            cat "${tmpfile}.merged" > "$mapper_json_file"
		...
		echo "Creating Initial Disk Layout $mapper_json_file File"
		if jq --argjson default_case '
			{
			  "case": null
			}' '. + $default_case' "$tmpfile" > "$mapper_json_file"; then
			
            if [[ -s "$mapper_json_file" ]]; then
                rm -f "$tmpfile"
                return 0
            else
#                echo "Error: JSON creation succeeded (exit status 0) but file is empty." >&2
                rm -f "$mapper_json_file" "$tmpfile"
                return 1
            fi
		else
#			echo "Error: JSON creation with 'case' failed (jq error)." >&2
			rm -f "$tmpfile"
            # Remove a zero-length or incomplete file if it exists
			test -s "$mapper_json_file" || rm -f "$mapper_json_file" 
			return 1
		fi
	fi
	
	if test -e "$mapper_json_file"; then
		cat "$mapper_json_file" > /tmp/disklayout_config.json
	else
		echo "ERROR - Unable to write $mapper_json_file to your disk.  Check to see if your permissions are correct."
	fi
}
...
# Go git the file "render_case.py"
	...
	# Do we have the file `render_case.py` ?
		...
		if test -e "/tmp/Multi-Report/render_case.py"; then
			...
			if [[ $Automatic_RenderCase_Update == "enable" ]] && [[ $UpdateRenderCaseAvailable == "true" ]] && [[ $Disklayout == "enable" ]]; then
				echo "      Automatic Update Enabled"
				update_RenderCase_script
			fi	
		fi
...
# Go git the file
		...
		echo " "
		if [[ $UpdateAvailable == "true" ]] || [[ $UpdateDriveAvailable == "true" ]] || [[ $UpdateSendemailAvailable == "true" ]] || [[ $UpdateRenderCaseAvailable == "true" && $Disklayout == "enable" ]]; then
			echo "UPDATE(s) AVAILABLE"
		elif [[ $UpdateAvailable == "update" ]] || [[ $UpdateDriveAvailable == "update" ]] || [[ $UpdateSendemailAvailable == "update" ]] || [[ $UpdateRenderCaseAvailable == "update" && $Disklayout == "enable" ]]; then
			echo "UPDATES MADE"
		fi
		echo " "
	fi
...
if [[ $Debug_Steps == "true" ]]; then echo "create_Email" | tee -a /tmp/multi_report_debug.txt; fi
	...
	if test -e "${logfile_header}"; then rm "${logfile_header}"; fi
		...
		if [[ $Monitor == "false" ]]; then
			...
			if [[ $UpdateRenderCaseAvailable == "true" || $UpdateRenderCaseAvailable == "update" ]] && [[ $Disklayout == "enable" ]]; then echo "<br>RENDER_CASE.PY UPDATE "$RendeerCase_Updated" --> V"$GitRenderCaseVersion" (Disk Layout)"; fi
...
#### CAN THE REST OF THIS FUNCTION BE MOVED INTO A SINGLE SECTION FOR ATTACHMENTS ?????   ######
	...
	if [[ "$dump_all" != "0" ]]; then
		...
		if [[ "$(echo $programver | grep -i "beta")" ]] || [[ $UpdateAvailable == "true" ]] || [[ $UpdateDriveAvailable == "true" ]] || [[ $UpdateSendemailAvailable == "true" ]]; then echo "<b><span style='color:darkred;'>"; fi
			...
			if [[ $UpdateRenderCaseAvailable == "true" ]] && [[ $Disklayout == "enable" ]]; then echo "<br>RENDER_CASE.PY (Disk Layout) UPDATE AVAILABLE --> "$GitSendemailVersion; fi
...
#		if [[ "$TempDisplay" == "&deg; C" ]]; then TempDisplay="° C"; fi
		...
		echo " "
		echo "###### Disk Layout ######"
		echo 'Disklayout="'$Disklayout'"			# BETA TESTING ONLY and SCALE ONLY.'
		echo 'MS_Outlook="'$MS_Outlook'"		# Set to "enable" to use the OUTLOOK compatable version.  Default="disable"'
		echo " "
...
if [[ "$UpdateRenderCaseAvailable" == "true" ]] && [[ $Disklayout == "enable" ]]; then
...
# Before we go and grab individual drive data, we need to initialize the Mapper Arrays
	...
	if test -e "$mapper_json_file"; then
#		echo $mapper_json_file" exists, reading the file...." | tee -a debug.txt
		mapper_read_json_file
	fi
...
if [[ "$Disklayout" == "enable" ]]; then
	...
	python3 render_case.py --config "$mapper_json_file" > /dev/null 2<&1
	exit_code=$?
	if [[ "$exit_code" != 0 ]]; then
		echo "<><><><><><><><><><><><><><><><><><><><><>"
