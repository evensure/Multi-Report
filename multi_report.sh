...
### Currently sendemail.py and case_render.py is maintained by Oxyde (oxyde1989@gmail.com)


# Version
### Changelog:

##############  CHECK EMAILS FOR POSSIBLE PROBLEMS WITH MR BETA BEFORE RELEASING IT.

# V3.28 (09 Dec 2025)
#
# Offical release of the Disk Layout feature.

# Beta 7 (08 Dec 2025)
# - Added High_Contrast variable to aid with difficulties with color.
# - Added restrictive logic so Disk Layout can only be used on a Linux system.
# - Added new get_zpool_devices function, for CORE only right now, but should work on SCALE as well.  Save for v4.0
# - Removed MS_Outlook setting since Oxyde updated 'render_case.py' to handle that function.
# - Updated the -config section to support Disk Layout.

# Beta 6 (21 Nov 2025)
# - Fixed the attachment to display correctly.

# Beta 4 (19 Nov 2025)
# - Hopefully fixed the email client HTML and Disk Layout issue. Holy Cow!  Complicated.  MS_Outlook variable no longer used.
# - Updated Multi-Report data collection.

# Beta3 (18 Nov 2025)
# - Updated Data collection to correct the lack of version numbers for the software during a -dump or -dev.
# - Centered the MS_Outlook Disk Layout (in render_case.py).
# - Added Disk Layout Address Change Warning for development.  This will create a file called disklayout_changes.log
# -- and tells you if the drive was removed or the address changed.
# -- How does the script works:
# -- When new drives are discovered with respect to the json file values, the drive data is recorded and no slot is assigned.
# -- The user next goes to the webpage to assign slots to the drives.
# -- On subsequent runs, if the drive is removed, a log entry will record it and the data except slot and address will be removed.
# -- If a drive is moved or a new one installed in a new slot, the slot number will remain empty until you go online to assign it.
# -- If a drive is moved or a new drives is installed into a previously addressed slot, no action is required as the slot should be assigned.
# --
# -- Problems:  The change between HDD/SSD and NVMe drives is likely to cause problems.  I'm not 100% certain ACPI addressing is fool-proof for NVMe.

# Beta2 (17 Nov 2025)
# - Added `-dev` for developer mode, no SMART Testing.

# V3.27Beta1 (16 Nov 2025)
#
# - Updated Disk Layout scripts, they are ready for more testing.
# - Change spacing for " MB" to "MB" and the other values.

# V3.27 (14 Nov 2025)
#
# - Adjusted to take into account invalid absurdly high temperature values.
# - Removed all references to `mr_sendemail.py` (originally expected to be removed in January 2026).
# - Partition files are now zipped together.
# - Fixed duplicate -dump files.
# - Added new POH value 'Years Days' (yd) per request.
# - Updated for another SanDisk variant - Wear Level, Test Age, Total Data Read/Written.

# The work in-between
# 14 Nov 2025 - Disk Layout still in Beta.
# 11 Nov 2025 - Fixed Disk Layout json file for pool name.
# 07 Nov 2025 - Fixed Disk Layout Chart placement and inclusion of missing tables.
# 07 Nov 2025 - Fixed SCSI Last Test Type and Time.
# 05 Nov 2025 - Fixed Uncorrectable Errors evaluation.
# 05 Nov 2025 - Beta Update 1 - Added Outlook Email compatibility.  case_render.py version 0.03 supports this change.

# V3.26 (04 Nov 2025)
#
# - Fixed ReAllocEvent setting, now it works again.
# - Added Drive Mapping. This is disabled during Beta Testing.  You may enable it by adding the
# -- value `Disklayout="enable"` in your multi_report_config.txt file.
# - Added json file creation check to verify the file is created.
#

# - Introducing Disk Layout Beta
#
# THINGS TO COMPLETE
#  DONE - 1. SMR UPDATE CHECKS - Disable if SMR checks are disabled.
#  DONE - 2. Check for updates for render_case.py
#  DONE - 3. Incorporate HTML into output, just attach to the end of the CHART section, before the Text Section.
#       -- Outlook has an issue displaying the grid.  Not a problem for the Beta testing.
#  DONE - 4. Remove the "case" in the json file.
#  DONE - 5. Hide render_case output.
#  DONE - 6. Write a correctly formatted json file.
#  DONE - 7. Add Disk Layout status (enabled/disabled) to the Text Report
#  DONE - 8. Include the "render_case" version number if disk_layout_Drive="enable"
#  DONE - 9. Fix no temps on first run when the json file is created.
#  DONE - 10. Add debug data collection for lsscsi and other commands.
#  DONE - 11. Test with Disklayout="disable" to ensure Beta testing crap isn't leaking through.
#  DONE - 12. Fixed growing config.json file.
#  DONE - 13. Fixed RepBytes Leak --> Added scrubRepBytesfull=""

#  WAITING CONFIRMATION -- 14.  Fixed Scrub Duration for "with 0" zpool output.

#  DONE - 15. Fixed initial json file creation to have `"case": null`.
#  DONE - 16. Changed configuration file name from "config.json" to "disklayout_config.json".
#  DONE - 17. Updated Cleanup Function, some old files and new files were left behind.
#  DONE - 18. Incorporated Resending Email Report for a few attempts (up to 60 minutes).


# V3.25 (Testing Version)
...
Check_For_Updates_Day_of_Week="All"		# Will only check for an update on this day of the week.  Default = "All" ("Mon","Tue","Wed","Thu","Fri","Sat","Sun","All","Month")
...
Automatic_Sendemail_Update="disable"	# WARNING !!!  This option will automatically update the Sendemail.py script if a newer version exists on GitHub with no user interaction. Default = "disable"
Automatic_SMR_Update="disable"			# For SMR
Automatic_RenderCase_Update="disable"	# This is for the Disk Layout
...
###### Disk Layout ######
Disklayout="enable"		# SCALE ONLY.  Options: "enable" or "disable".
High_Contrast="false"		# This will produce a high contrast output.  Default="false".
...
HDD_Cur_Pwr_Max_Temp_Ovrd="disable"	# HDD Max Drive Temp Override. This value when "enable" will NOT alarm on any Current Power Cycle Max Temperature Limit.
SSD_Cur_Pwr_Max_Temp_Ovrd="disable"	# SSD Max Drive Temp Override. This value when "enable" will NOT alarm on any Current Power Cycle Max Temperature Limit.
NVM_Cur_Pwr_Max_Temp_Ovrd="disable"	# NVM Max Drive Temp Override. This value when "enable" will NOT alarm on any Current Power Cycle Max Temperature Limit.
...
PowerTimeFormat="h"				# Format for power-on hours string, valid options are "ymdh", "ymd", "ym", "yd", "y", or "h" (year month day hour).
...
lastTestTypeHoursIdent="hrs"	# Test to follow power on hours numbers (hrs or yd).  Default = "hrs".
...
IgnoreSeekError="disable"	# Set to "enable" to ignore all Seek Error Rate/Health errors.  Default is "disable".
IgnoreReadError="disable"	# Set to "enable" to ignore all Seek Error Rate/Health errors.  Default is "disable".
...
HDD_Total_Data_Written_30day_Title="Total Data Written 30 Days"
...
SSD_Total_Data_Written_30day_Title="Total Data Written 30 Days"
...
NVM_Total_Data_Written_30day_Title="Total Data Written 30 Days"
...
Silent="enable"                            # When "enable" only error messages will be output to the stdout. Set to "disable" if troubleshooting a testing problem.
...
CurrentFilename="multi_report_v3.28_2025_12_09.txt"
valid_config_version_date="2025-12-09"	# Configuration file valid date
...
disk_layout_version=0.3							# Format Major.Minor (Changed 23 Nov 2025)

# Assign Variables
disk_layout_html_file="$SCRIPT_DIR/case_email_snippet.html"
disk_layout_json_file="$SCRIPT_DIR/disklayout_config.json"
tmpfile="/tmp/mapper.tmp"


# Define arrays
declare -a disk_layout_slot						# The Grid Slot number (probably replaces the Bay value)
declare -a disk_layout_name						# Drive ID
declare -a disk_layout_type						# Drive Type (hdd/nvme)
declare -a disk_layout_serial					# Drive Serial Number
declare -a disk_layout_model						# Drive Model
declare -a disk_layout_capacity					# Drive Capacity
declare -a disk_layout_uuid						# Drive UUID (if exist)
declare -a disk_layout_addr						# HCTL of the drive data connector
declare -a disk_layout_bay						# The drive bay/slot number assigned to the HCTL address
declare -a disk_layout_pool						# The drive is part of this pool

# These two need to be set from within Multi-Report data collection section.
# -- Scroll through all array values looking for the serial number, then apply the two values below.
declare -a disk_layout_color						# The drive color will change to indicate a problem
declare -a disk_layout_temperature				# The current drive temperature
...
disk_layout_join_chart() {
if [[ $Debug_Steps == "true" ]]; then echo "disk_layout_join_chart" | tee -a /tmp/multi_report_debug.txt; fi
output=$logfile
page1="page1.html"
page3="page3.html"

page_emclient="Disk_layout.html" # This holds the content from render_email_snippet
...
echo "<style>"
echo "body { background-color:#f8f8f8; font-family:'Times New Roman', serif; margin:20px; }"
...
echo "<h2>Drive Layout Visualization</h2>"
	
if [[ ${disk_layout_slot[0]} == "X" || ${disk_layout_slot[0]} == "" ]]; then
	echo "YOUR CONFIG FILE IS NOT COMPLETE."
	echo "Visit https://joeschmuck.github.io/Multi-Report_Hub/ to assign your drives to a location."
fi
	
#echo ""
echo "<table role=\"presentation\" border=\"0\" cellpadding=\"0\" cellspacing=\"0\" width=\"100%\" style=\"mso-table-lspace:0pt; mso-table-rspace:0pt; border-collapse:collapse; display:table; margin-top: -15px !important;\" align=\"left\">"
echo "<tr><td style=\"display:block; mso-hide:all;\">"
cat "$page_emclient"
echo ""
echo "</td></tr>"
echo "</table>"
...
# --- END DUAL CLIENT CONTENT ---

echo "<hr style='margin:40px 0;'>"

if [[ -f "$page3" ]]; then
	start_line=$(grep -n -m1 '<pre style="font-size:21; font-family: courier new"' "$page3" | cut -d: -f1 || true)
...
if test -e "page1.html"; then rm "page1.html"; fi
if test -e "page3.html"; then rm "page3.html"; fi

}


# The other functions (split_html and disk_layout_join_html) do NOT need modification.
...
# Use awk to split at the "Multi-Report Text Section"
awk -v out1="$part1" -v out2="$part2" '
{
  if ($0 ~ /<b>Multi-Report Text Section/) {
    close(out1)
    print > out2          # include this line in page3
    while (getline) print > out2
    exit
  }
  print > out1
}' "$input_file"
...
disk_layout_join_html() {
if [[ $Debug_Steps == "true" ]]; then echo "disk_layout_join_html"; fi
...
disk_layout_set_values() {
if [[ $Debug_Steps == "true" ]]; then echo "disk_layout_set_values"; fi
# This will load the Temp and Color values into the arrays.

if [[ "$deviceStatusColor" == "$whtColor" || "$deviceStatusColor" == "$altColor" ]]; then temp_color="green"
elif [[ "$deviceStatusColor" == "$critColor" ]]; then temp_color="red"
elif [[ "$deviceStatusColor" == "$warnColor" ]]; then temp_color="orange"
elif [[ "$deviceStatusColor" == "$yellowColor" ]]; then temp_color="yellow"
else
	temp_color="none"
fi
...
array_index=0  # Tracks the index for the arrays (only increments for valid drives)
array_size=${#disk_layout_serial[@]}

# Loop through ALL lines looking for a serial number match
while [[ $array_index -le $array_size ]]; do
	# Read each array value and locate a match.
	if [[ "${disk_layout_serial[array_index]}" == "$serial" ]]; then
		disk_layout_temperature[array_index]=$temp
		disk_layout_color[array_index]=$temp_color
		return
	fi
    (( array_index++ ))
done
...
disk_layout_collect_drive_data() {
# Commands we use in this function:  lsscsi, lsblk, 
if [[ $Debug_Steps == "true" ]]; then echo "disk_layout_collect_drive_data"; fi
# Initialize temporary arrays for new data collection
live_hardware_name=()
live_hardware_type=()
live_hardware_serial=()
live_hardware_model=()
live_hardware_capacity=()
live_hardware_uuid=()
live_hardware_addr=()
live_hardware_pool=()
declare -gA live_serial_to_index_map # Clear and set map globally
declare -gA live_addr_to_serial      # Clear and set map globally

lsscsi_line=1
array_index=0
total_lsscsi_count=$(lsscsi | wc -l)

while [[ $lsscsi_line -le $total_lsscsi_count ]]; do

    disk_layout_DEVICE_NAME=$(lsscsi | awk '{print $NF}' | cut -d'/' -f3 | head -n ${lsscsi_line} | tail -n 1)
	disk_layout_DEVICE_DISK=$(lsscsi | awk '{print $2}' | head -n ${lsscsi_line} | tail -n 1)
	
    if [[ -z "$disk_layout_DEVICE_NAME" ]] || [[ "$disk_layout_DEVICE_DISK" != "disk" ]]; then
        (( lsscsi_line++ ))
        continue
    fi

    disk_layout_MODEL_NAME=$(lsblk -n -d -o MODEL "/dev/${disk_layout_DEVICE_NAME}")

    if echo "$disk_layout_MODEL_NAME" | grep -qvi "vmware\|virtual"; then
        # Collect live hardware data into temporary arrays
        live_hardware_name[array_index]="$disk_layout_DEVICE_NAME"
        live_hardware_model[array_index]="$disk_layout_MODEL_NAME"
        current_serial=$(lsblk -n -d -o SERIAL "/dev/${disk_layout_DEVICE_NAME}")
		live_hardware_serial[array_index]="$current_serial"
        disk_layout_CAP_ADDR_LINE=$(lsscsi -s | head -n ${lsscsi_line} | tail -n 1)
        live_hardware_capacity[array_index]=$(echo "$disk_layout_CAP_ADDR_LINE" | sed 's/\( \/dev\/[a-z]\+\)$//' | awk '{print $NF}')
		pool_names="$(zpool status | grep "pool:" | awk -F" " '{print $2}')"

		for temp_pool in $pool_names; do
			# Looking for match with pool name, result is UUID
			temp_pool_uuid_match="$(lsblk -n -o LABEL,PARTUUID /dev/${disk_layout_DEVICE_NAME} | grep "$temp_pool" | awk -F" " '{print $2}')"
			if [[ "$temp_pool_uuid_match" != "" ]]; then break; else temp_pool=""; temp_pool_uuid_match=""; fi
		done
		live_hardware_uuid[array_index]=$temp_pool_uuid_match

		if [[ -n "$temp_pool" ]]; then
			temp_pool_name=$(lsblk -n -o LABEL,PARTUUID "/dev/${disk_layout_DEVICE_NAME}" | grep "${live_hardware_uuid[array_index]}" | grep "$temp_pool_uuid_match" | awk -F" " '{print $1}')
		else
			temp_pool_name=""
		fi

		if [[ -z "$temp_pool_name" ]]; then
            live_hardware_pool[array_index]="No Pool Assigned"
        else
		    live_hardware_pool[array_index]="$temp_pool_name"
        fi

        if [[ ${live_hardware_name[array_index]} == *"nvm"* ]]; then
            temp_name=$(basename "${live_hardware_name[array_index]}")
            current_addr=$(basename "$(readlink -f /sys/block/$temp_name/device/device)" 2>/dev/null)
            live_hardware_type[array_index]="nvme"
        else
            current_addr=$(echo "$disk_layout_CAP_ADDR_LINE" | cut -d'[' -f2 | cut -d']' -f1)
            live_hardware_type[array_index]="hdd"
        fi
        live_hardware_addr[array_index]="$current_addr"

        # Populate the live lookup maps
        live_serial_to_index_map["$current_serial"]="$array_index"
        live_addr_to_serial["$current_addr"]="$current_serial"

		if [[ $Debug_Steps == "true" ]]; then
			echo
			echo "disk_layout_DEVICE_DISK="$disk_layout_DEVICE_DISK
			echo "live_hardware_name[array_index]="${live_hardware_name[array_index]}
			echo "live_hardware_model[array_index]="${live_hardware_model[array_index]}
			echo "live_hardware_serial[array_index]="${live_hardware_serial[array_index]}
			echo "live_hardware_capacity[array_index]="${live_hardware_capacity[array_index]}
			echo "live_hardware_uuid[array_index]="${live_hardware_uuid[array_index]}
			echo "live_hardware_pool[array_index]="${live_hardware_pool[array_index]}
			echo "======================================================================"
		fi
        (( array_index++ ))
    fi
...
disk_layout_read_json_file() {
if [[ $Debug_Steps == "true" ]]; then echo "disk_layout_read_json_file"; fi	

	# Generate debug data, just in case we need it.
	lsscsi -s > /tmp/lsscsi.txt
	blkid -s PARTUUID -s LABEL > /tmp/blkid.txt
	lsblk -J -a -O > /tmp/lsblk.txt

if ! test -e "$disk_layout_json_file"; then 
    if [[ $Debug_Steps == "true" ]]; then echo "JSON file not found. Skipping read."; fi
    return 1
fi
	
# Initialize arrays for old data read
old_disk_layout_slot=()
old_disk_layout_addr=()
old_disk_layout_serial=()
old_disk_layout_pool=()
old_disk_layout_color=()
old_disk_layout_temperature=()
declare -gA fixed_addr_to_slot_map # Ensure the map is globally available and cleared

# Count entries
total_bays_count=$(grep -o "slot" "$disk_layout_json_file" | wc -l)

lsscsi_line=0
while [[ $lsscsi_line -lt $total_bays_count ]]; do
    # Read all old values
    old_disk_layout_slot[lsscsi_line]=$(cat "$disk_layout_json_file" | jq -Mre '.bays['${lsscsi_line}'].slot | values')
    old_disk_layout_addr[lsscsi_line]=$(cat "$disk_layout_json_file" | jq -Mre '.bays['${lsscsi_line}'].address | values')
    old_disk_layout_serial[lsscsi_line]=$(cat "$disk_layout_json_file" | jq -Mre '.bays['${lsscsi_line}'].serial | values')
    old_disk_layout_pool[lsscsi_line]=$(cat "$disk_layout_json_file" | jq -Mre '.bays['${lsscsi_line}'].pool | values')
    old_disk_layout_color[lsscsi_line]=$(cat "$disk_layout_json_file" | jq -Mre '.bays['${lsscsi_line}'].drive_color | values')
    old_disk_layout_temperature[lsscsi_line]=$(cat "$disk_layout_json_file" | jq -Mre '.bays['${lsscsi_line}'].drive_temp | values')
    
    # Create a fixed address-to-slot mapping from the old JSON
    local addr="${old_disk_layout_addr[lsscsi_line]}"
    local slot="${old_disk_layout_slot[lsscsi_line]}"
    
    if [[ -n "$addr" ]] && [[ -n "$slot" ]]; then
        fixed_addr_to_slot_map["$addr"]="$slot"
    fi
    
    (( lsscsi_line++ ))
done

if [[ ${old_disk_layout_slot[0]} == "" ]]; then old_disk_layout_slot[0]="X"; fi
}


disk_layout_reconcile_data() {
    if [[ $Debug_Steps == "true" ]]; then echo "disk_layout_reconcile_data"; fi

    local old_config_count=${#old_disk_layout_serial[@]}
    ...
    disk_layout_type=()
    disk_layout_model=()
    disk_layout_capacity=()
    disk_layout_uuid=()
    disk_layout_pool=()
    disk_layout_color=()
    disk_layout_temperature=()

    local LOGFILE="$SCRIPT_DIR/disklayout_changes.log"

    # The primary loop must be over the OLD configuration to ensure all previously known slots/addresses are retained.
    for (( old_idx=0; old_idx < old_config_count; old_idx++ )); do

        local old_addr="${old_disk_layout_addr[old_idx]}"
        local old_slot="${old_disk_layout_slot[old_idx]}"
        local old_serial="${old_disk_layout_serial[old_idx]}"

        local current_index_for_addr=""
        local current_serial_at_addr="${live_addr_to_serial[$old_addr]}"

        # 1. Check if the address is currently occupied by a live drive
        if [[ -n "$current_serial_at_addr" ]]; then

            current_index_for_addr="${live_serial_to_index_map[$current_serial_at_addr]}"

            # Pull live data
            disk_layout_addr[old_idx]="${live_hardware_addr[current_index_for_addr]}"
            disk_layout_name[old_idx]="${live_hardware_name[current_index_for_addr]}"
            disk_layout_type[old_idx]="${live_hardware_type[current_index_for_addr]}"
            disk_layout_serial[old_idx]="${live_hardware_serial[current_index_for_addr]}"
            disk_layout_model[old_idx]="${live_hardware_model[current_index_for_addr]}"
            disk_layout_capacity[old_idx]="${live_hardware_capacity[current_index_for_addr]}"
            disk_layout_uuid[old_idx]="${live_hardware_uuid[current_index_for_addr]}"
            disk_layout_pool[old_idx]="${live_hardware_pool[current_index_for_addr]}"

            disk_layout_slot[old_idx]="$old_slot"

            # Restore persistent metadata (only if same serial existed before)
            if [[ -n "${live_serial_to_index_map[$old_serial]}" ]]; then
                disk_layout_color[old_idx]="${old_disk_layout_color[old_idx]}"
                disk_layout_temperature[old_idx]="${old_disk_layout_temperature[old_idx]}"
            else
                disk_layout_color[old_idx]=""
                disk_layout_temperature[old_idx]=""
            fi

            if [[ $Debug_Steps == "true" ]]; then
                if [[ ${old_slot} != "" ]]; then
                    echo "Slot ${old_slot} occupied by drive S/N: ${current_serial_at_addr}."
                else
                    echo "Slot 'not assigned' to drive S/N: ${current_serial_at_addr}."
                fi
            fi

        else
            # Drive is NOT PRESENT at this address
            # Rule: Retain Slot and Address, but null out remaining data.
            disk_layout_slot[old_idx]="$old_slot"
            disk_layout_addr[old_idx]="$old_addr"
            disk_layout_serial[old_idx]=""
            disk_layout_name[old_idx]=""
            disk_layout_type[old_idx]=""
            disk_layout_model[old_idx]=""
            disk_layout_capacity[old_idx]=""
            disk_layout_uuid[old_idx]=""
            disk_layout_pool[old_idx]=""
            disk_layout_color[old_idx]=""
            disk_layout_temperature[old_idx]=""

            if [[ $Debug_Steps == "true" ]]; then
                echo "Slot ${old_slot} (Address ${old_addr}) is empty. Drive data nulled."
            fi

            # Log drive removal (slot becomes empty) if previously there was a serial recorded
            if [[ -n "$old_serial" ]]; then
                local ts
                ts=$(date '+%Y-%m-%d %H:%M:%S')
                echo "$ts  SLOT=$old_slot  DRIVE_REMOVED  ADDR=$old_addr  SERIAL=$old_serial" >> "$LOGFILE"
                if [[ $Debug_Steps == "true" ]]; then
                    echo "LOG: SLOT=$old_slot DRIVE_REMOVED ADDR=$old_addr SERIAL=$old_serial"
                fi
            fi
			
			# If Slot was never assigned then we need to remove the entire entry.
			if [[ ${old_slot} == "" ]]; then
				disk_layout_addr[old_idx]=""
				if [[ $Debug_Steps == "true" ]]; then
					echo "Address ${old_addr} nulled."
				fi
			fi
        fi
        ...
        #
        # ----------------------------------------------------
        # Log if ADDRESS CHANGED for this SLOT (previous run → current run)
        # ----------------------------------------------------
        #
        local new_addr="${disk_layout_addr[old_idx]}"

        # Log when slot existed previously AND stored address changed compared to current computed address
        # This captures manual edits to the JSON or other address mismatches.
        if [[ -n "$old_slot" && "$old_addr" != "$new_addr" ]]; then
            local ts
            ts=$(date '+%Y-%m-%d %H:%M:%S')
            echo "$ts  SLOT=$old_slot  ADDRESS_CHANGED  OLD=$old_addr  NEW=$new_addr  SERIAL=$old_serial" >> "$LOGFILE"
            if [[ $Debug_Steps == "true" ]]; then
                echo "LOG: Slot $old_slot address changed (old: $old_addr new: $new_addr)"
            fi
        fi
        # ----------------------------------------------------
    ...
    # Account for entirely NEW drives in NEW addresses
    for (( live_idx=0; live_idx < ${#live_hardware_addr[@]}; live_idx++ )); do
        local current_addr="${live_hardware_addr[live_idx]}"
        local is_new_address=true

        for old_addr_check in "${old_disk_layout_addr[@]}"; do
            if [[ "$current_addr" == "$old_addr_check" ]]; then
                is_new_address=false
                break
            fi
        done

        if [[ "$is_new_address" == true ]]; then
            local new_idx=${#disk_layout_slot[@]}

            disk_layout_slot[new_idx]="" # New slots are unassigned
            disk_layout_addr[new_idx]="${live_hardware_addr[live_idx]}"
            disk_layout_name[new_idx]="${live_hardware_name[live_idx]}"
            disk_layout_type[new_idx]="${live_hardware_type[live_idx]}"
            disk_layout_serial[new_idx]="${live_hardware_serial[live_idx]}"
            disk_layout_model[new_idx]="${live_hardware_model[live_idx]}"
            disk_layout_capacity[new_idx]="${live_hardware_capacity[live_idx]}"
            disk_layout_uuid[new_idx]="${live_hardware_uuid[live_idx]}"
            disk_layout_pool[new_idx]="${live_hardware_pool[live_idx]}"
            disk_layout_color[new_idx]=""
            disk_layout_temperature[new_idx]=""

            if [[ $Debug_Steps == "true" ]]; then
                echo "New drive/address detected (${current_addr}). Appending to list."
            fi

            # Log new entry (new drive or moved drive)
            local ts
            ts=$(date '+%Y-%m-%d %H:%M:%S')
            echo "$ts  NEW_ENTRY  ADDR=${live_hardware_addr[live_idx]}  SERIAL=${live_hardware_serial[live_idx]}  SLOT_ASSIGNED=none" >> "$LOGFILE"
            if [[ $Debug_Steps == "true" ]]; then
                echo "LOG: NEW_ENTRY ADDR=${live_hardware_addr[live_idx]} SERIAL=${live_hardware_serial[live_idx]}"
            fi
        fi
    done
...
disk_layout_write_json_file() {
if [[ $Debug_Steps == "true" ]]; then echo "disk_layout_write_json_file"; fi
    local disk_layout_name_count=${#disk_layout_slot[@]} # Use slot count now, as it reflects all entries
    local disk_layout_read_version=$disk_layout_version
    local disk_layout_read_version_update="$(date)"

    # Check for empty slot value after reconciliation
	local first_slot="${disk_layout_slot[0]}"
    if [[ "${disk_layout_slot[0]}" == "X" ]]; then
        echo
        echo
        echo "<><><><><><><><><><><><><><><><><><><><><><><><><><><><><><>"	
        echo "<>         YOUR CONFIG FILE IS NOT COMPLETE.             <>"
        echo "<>   Visit https://joeschmuck.github.io/Multi-Report_Hub/  <>"
        echo "<>         to assign your drives to a location.          <>"
        echo "<><><><><><><><><><><><><><><><><><><><><><><><><><><><><><>"
        echo
        echo
    fi
    
    {
        ...
        echo '  "version": ['
        echo '    {'
        echo "      \"version\": \"${disk_layout_read_version}\","
        echo "      \"last_updated\": \"${disk_layout_read_version_update}\""
        echo '    }'
        echo '  ],'
		echo "	\"high_contrast\": \"${High_Contrast}\","
        echo '  "bays": ['

        local first_entry=true
        for ((count=0; count<disk_layout_name_count; count++)); do
            if [[ -z "${disk_layout_addr[count]}" ]]; then
                continue # Skip entries without an address (shouldn't happen here)
            fi
            ...
            cat <<EOF
    {
      "slot": "${disk_layout_slot[count]}",
      "address": "${disk_layout_addr[count]}",
      "pool": "${disk_layout_pool[count]}",
      "drive_id": "${disk_layout_name[count]}",
	  "type": "${disk_layout_type[count]}",
      "serial": "${disk_layout_serial[count]}",
      "model": "${disk_layout_model[count]}",
      "capacity": "${disk_layout_capacity[count]}",
      "uuid": "${disk_layout_uuid[count]}",
      "drive_color": "${disk_layout_color[count]}",
      "drive_temp": "${disk_layout_temperature[count]}"
    }
EOF
        done
        echo
    ...
    if test -e "$disk_layout_json_file"; then
        if jq --slurp '.[0] * { case: .[1].case }' "$tmpfile" "$disk_layout_json_file" > "${tmpfile}.merged"; then
            cat "${tmpfile}.merged" > "$disk_layout_json_file"
            rm -f "$tmpfile" "${tmpfile}.merged"
            return 0
        else
            echo "Error: JSON merge failed. Keeping original file." >&2
            rm -f "$tmpfile" "${tmpfile}.merged"
            return 1
        fi
    else
        echo "Creating Initial Disk Layout $disk_layout_json_file File"
        if jq --argjson default_case '
            {
              "case": null
            }' '. + $default_case' "$tmpfile" > "$disk_layout_json_file"; then
            
            if [[ -s "$disk_layout_json_file" ]]; then
                rm -f "$tmpfile"
                return 0
            else
                rm -f "$disk_layout_json_file" "$tmpfile"
                return 1
            fi
        else
            rm -f "$tmpfile"
            test -s "$disk_layout_json_file" || rm -f "$disk_layout_json_file"
            return 1
        fi
    fi
}
...
#<<<<<<<<<<<<<<<<<<<<< END OF MAPPER FUNCTIONS >>>>>>>>>>>>>>>>>>>

#################### FUNCTIONS ####################

get_location () {
if [[ $Debug_Steps == "true" ]]; then echo "get_location"; fi
...
SMART_check_status ()
	{
	if [[ $Debug_Steps == "true" ]]; then echo "SMART_check_status"; fi
	# This will check if a drive has SMART disabled.
	shopt -s nocasematch 
    if [[ $softver != "Linux" ]]; then
		smart_act=$(sysctl -n kern.disks)
	else
		smart_act=$(fdisk -l 2>/dev/null | grep "Disk /dev/sd" | cut -c 11-15 | cut -d ":" -f1 | tr '\n' ' ') > /dev/null 2<&1
	fi
...
if [[ $Debug_Steps == "true" ]]; then echo "SMART_activate"; fi
	# Enable SMART
	shopt -s nocasematch 
    if [[ $softver != "Linux" ]]; then
		smart_act=$(sysctl -n kern.disks)
...
if [[ $Debug_Steps == "true" ]]; then echo "run_smart_data_command"; fi
...
if [[ $Debug_Steps == "true" ]]; then echo "get_API_Drives_and_tar"; fi
#
#  This procedure will:
...
# Updated section for possible BSD issues
		total=$(wc -l < /tmp/attachment.json)
		keep=$((total - 2))
		if [ "$keep" -gt 0 ]; then
			updating_attachment=$(head -n "$keep" /tmp/attachment.json)
		else
			updating_attachment=""
		fi
########################################
...
if [[ $Debug_Steps == "true" ]]; then echo "update_smr" | tee -a /tmp/multi_report_debug.txt; fi
	...
	echo "      - Downloading new script file"
	if [[ "$(curl -is https://github.com | head -n 1)" ]]; then
		...
		if test -e $SCRIPT_DIR"/smr-check.sh"; then
			...
			VersionSMRFilename=0
		fi
		echo " "
		echo "      - SMR-Check current version is: "$VersionSMRFilename
		echo "      - The GitHub version is: "$GitSMRVersion 
		echo " "
		VersionSMRFilename=$GitSMRVersion
		if [[ $Automatic_SMR_Update != "enable" ]]; then
		...
		if [[ $Keyboard_yn == "y" ]] || [[ $Keyboard_yn == "Y" ]]; then
			...
			cp /tmp/Multi-Report/smr-check.sh $SCRIPT_DIR"/smr-check.sh"
			chmod 755 $SCRIPT_DIR"/smr-check.sh" > /dev/null 2<&1
			echo " "
			echo "      - Script updated"
			echo " "
			SMR_Updated="UPDATED: V"$VersionSMRFilename
			UpdateSMRAvailable="update"
			sleep 1
		else
			echo "Aborted"
			SMR_Updated="ABORTED: V"$VersionSMRFilename
			if test -e "/tmp/Multi-Report"; then rm -R "/tmp/Multi-Report"; fi
		fi
	else
	...
	if [[ $Automatic_SMR_Update == "disable" ]] && [[ $update_all == "" ]]; then 
		echo "Exiting..."
		exit 0
	fi
...
if [[ $Debug_Steps == "true" ]]; then echo "update_MR_script" | tee -a /tmp/multi_report_debug.txt; fi
	...
	if [[ "$(curl -is https://github.com | head -n 1)" ]]; then
		...
		echo "      - The GitHub version is: "$GitVersion 
		echo " "
...
# FOR OXYDE GITHUB
		...
		echo "      - The GitHub version is: "$GitSendemailVersion 
		echo " "
	...
	update_RenderCase_script () {
if [[ $Debug_Steps == "true" ]]; then echo "update_RenderCase_script" | tee -a /tmp/multi_report_debug.txt; fi
	echo "      - Update Script Routine"
	echo "      - Removing Old Script Source if it exists"
	if test -e "/tmp/Multi-Report"; then rm -R "/tmp/Multi-Report"; fi
	echo "      - Downloading new script file"
	if [[ "$(curl -is https://github.com | head -n 1)" ]]; then
		...
		cd /tmp/Multi-Report || exit 3
# FOR OXYDE GITHUB
		curl -LJOs https://raw.githubusercontent.com/JoeSchmuck/Multi-Report_Hub/refs/heads/main/docs/DiskLayout_Editor/render_case.py
		)
		GitRenderCaseVersion=$(cat /tmp/Multi-Report/render_case.py | grep "##### V" | cut -d 'V' -f2 | tr -d ' ')
		if test -e $SCRIPT_DIR"/render_case.py"; then
			VersionRenderCaseFilename=$(cat $SCRIPT_DIR"/render_case.py" | grep "##### V" | cut -d 'V' -f2 | tr -d ' ')
		else
			VersionRenderCaseFilename="Not Installed"
		fi
		echo " "
		echo "      - RenderCase.py current version is: "$VersionRenderCaseFilename
		echo "      - The GitHub version is: "$GitRenderCaseVersion 
		echo " "
		VersionRenderCaseFilename=$GitRenderCaseVersion

		if [[ $Automatic_RenderCase_Update != "enable" ]]; then
			echo "Enter 'y' to commit or any other key to abort."
			read Keyboard_yn
		else
			Keyboard_yn="y"
		...
		if [[ $Keyboard_yn == "y" ]] || [[ $Keyboard_yn == "Y" ]]; then
			echo "      - Updating Script..."
			cp /tmp/Multi-Report/render_case.py $SCRIPT_DIR"/render_case.py"
			echo " "
			echo "      - Your script has been updated."
			echo " "
			RenderCase_Updated="UPDATED: V"$VersionRenderCaseFilename
			UpdateRenderCaseAvailable="update"
			sleep 1
		else
			echo "Aborted"
			RenderCase_Updated="ABORTED: V"$VersionRenderCaseFilename
			if test -e "/tmp/Multi-Report"; then rm -R "/tmp/Multi-Report"; fi
		fi
	else
		echo "GitHub is Not Available"
	...
	# Exit if not an automatic update
	if [[ $Automatic_RenderCase_Update == "disable" ]] && [[ $update_all == "" ]]; then 
		echo "Exiting..."
		exit 0
	fi
	}
...
if [[ $Debug_Steps == "true" ]]; then echo "read_current_versions" | tee -a /tmp/multi_report_debug.txt; fi

	# Multi-Report Installed
		VersionFilename="$(cat $SCRIPT_DIR"/multi_report.sh" | grep "##### Version" | cut -d 'n' -f2 | cut -d '_' -f 1 | head -1)"
	...
	# Drive-Selftest Installed
		if test -e $SCRIPT_DIR"/drive_selftest.sh"; then
			...
			VersionDriveFilename="drive_selftest.sh is not on your system."
		fi
		
	# SMR Check Installed
		if test -e $SCRIPT_DIR"/smr-check.sh"; then
			VersionSMRFilename="$(cat $SCRIPT_DIR"/smr-check.sh" | grep "##### Version" | cut -d 'n' -f2 | tr -d ' ' | head -1)"
			if [[ $VersionSMRFilename == "" ]]; then VersionSMRFilename=0; fi
			...
			VersionSMRFilename="smr-check.sh is not on your system."
		fi
	
	# Sendemail.py Installed
		if test -e $SCRIPT_DIR"/sendemail.py"; then
	...
	# Render_Case Installed
			
		if test -e $SCRIPT_DIR"/render_case.py"; then
			VersionRenderCaseFilename="$(cat $SCRIPT_DIR"/render_case.py" | grep "##### V" | cut -d 'V' -f2 | tr -d ' ' | head -1)"
			MajorRenderCaseVersionFilename="$(echo $VersionRenderCaseFilename | cut -d '.' -f1)"
			MinorRenderCaseVersionFilename="$(echo $VersionRenderCaseFilename | cut -d '.' -f2)"
			convert_to_decimal $MajorRenderCaseVersionFilename
			MajorRenderCaseVersionFilename=$Return_Value
			convert_to_decimal $MinorRenderCaseVersionFilename
			MinorRenderCaseVersionFilename=$Return_Value
		else
			VersionRenderCaseFilename="render_case.py not installed on your system."
		fi		
...
if [[ $Debug_Steps == "true" ]]; then echo "checkforupdate" | tee -a /tmp/multi_report_debug.txt; fi
	...
	if [[ "$(curl -is https://github.com | head -n 1)" ]]; then
		...
		if test -e "/tmp/Multi-Report/multi_report.txt"; then
			...
			echo -n "   Multi-Report - Installed: "$VersionFilename", GitHub: "$GitVersion
...
# Go git the file "drive_selftest.txt"
		...
		if test -e "/tmp/Multi-Report/drive_selftest.txt"; then
			...
			echo -n "   Drive-Selftest - Installed: "$VersionDriveFilename", GitHub: "$GitDriveVersion
...
# Go git the file
		if [[ $truenas_sendmail_support == "No" ]]; then
			...
			if test -e "/tmp/Multi-Report/sendemail.py"; then
				...
				echo -n "   Sendemail - Installed: "$VersionSendemailFilename", GitHub: "$GitSendemailVersion
			...
			echo "   Sendemail - Not Required on CORE"
		fi
...
# Go git the file "smr-check.sh"
		if [[ $SMR_Enable == "enable" ]]; then
			...
			if test -e "/tmp/Multi-Report/smr-check.sh"; then
				...
				echo -n "   SMR-Check - Installed: "$VersionSMRFilename", GitHub: "$GitSMRVersion
				...
				if [[ $Automatic_SMR_Update == "enable" ]] && [[ $UpdateSMRAvailable == "true" ]] && [[ $SMR_Enable == "enable" ]]; then
					echo "      Automatic Update Enabled"
					update_smr
				fi					
			else
...
# Go git the file "render_case.py"
	if [[ $Disklayout == "enable" ]] && [[ $softver == "Linux" ]]; then
	# Do we have the file `render_case.py` ?
		(
			cd /tmp || exit 3
			if ! test -e "Multi-Report"; then
				mkdir Multi-Report
		...
		if test -e "/tmp/Multi-Report/render_case.py"; then
			GitRenderCaseVersion=$(cat "/tmp/Multi-Report/render_case.py" | grep "##### V" | cut -d 'V' -f2 | tr -d ' ' | head -1)
			GitRenderCaseMajorVersionNumber="$(echo $GitRenderCaseVersion | cut -d '.' -f1)"		
			GitRenderCaseMinorVersionNumber="$(echo $GitRenderCaseVersion | cut -d '.' -f2)"
			convert_to_decimal $GitRenderCaseMajorVersionNumber
			GitRenderCaseMajorVersionNumber=$Return_Value
			convert_to_decimal $GitRenderCaseMinorVersionNumber
			GitRenderCaseMinorVersionNumber=$Return_Value		
			...
			if test -e $SCRIPT_DIR"/render_case.py"; then
				...
				VersionRenderCaseFilename="Nothing At All, Fetching..."
				Automatic_RenderCase_Update="enable"
			fi
			
			RenderCase_Updated="AVAILABLE: V"$VersionRenderCaseFilename

			echo -n "   Disk Layout (render_case) - Installed: "$VersionRenderCaseFilename", GitHub: "$GitRenderCaseVersion
			if [[ $GitRenderCaseMajorVersionNumber -gt $MajorRenderCaseVersionFilename ]] && [[ $VersionRenderCaseFilename != "" ]]; then
				echo " - Major Update"
				UpdateRenderCaseAvailable="true"
				rm /tmp/Multi-Report/render_case.py
			elif [[ $GitRenderCaseMajorVersionNumber -eq $MajorRenderCaseVersionFilename ]] && [[ $GitRenderCaseMinorVersionNumber -gt $MinorRenderCaseVersionFilename ]]; then
				echo " - Minor Update"
				UpdateRenderCaseAvailable="true"
				rm /tmp/Multi-Report/render_case.py
			else
			...
			if [[ $Automatic_RenderCase_Update == "enable" ]] && [[ $UpdateRenderCaseAvailable == "true" ]] && [[ $Disklayout == "enable" ]] && [[ $softver == "Linux" ]]; then
				echo "      Automatic Update Enabled"
				update_RenderCase_script
			fi	
		fi
...
# Go git the file
		...
		echo " "
		if [[ $UpdateAvailable == "true" ]] || [[ $UpdateDriveAvailable == "true" ]] || [[ $UpdateSendemailAvailable == "true" ]] || [[ $UpdateRenderCaseAvailable == "true" && $Disklayout == "enable" && $softver == "Linux" ]]; then
			echo "UPDATE(s) AVAILABLE"
		elif [[ $UpdateAvailable == "update" ]] || [[ $UpdateDriveAvailable == "update" ]] || [[ $UpdateSendemailAvailable == "update" ]] || [[ $UpdateRenderCaseAvailable == "update" && $Disklayout == "enable" && $softver == "Linux" ]]; then
			echo "UPDATES MADE"
		fi
		echo " "
	fi
...
if [[ $Debug_Steps == "true" ]]; then echo "cleanup_files" | tee -a /tmp/multi_report_debug.txt; fi
	...
	f=(/tmp/*error_log.txt)
	if [[ -f "${f[0]}" ]]; then rm /tmp/*error_log.txt; fi	
	...
	if test -e "/tmp/smr-check.sh"; then rm "/tmp/smr-check.sh"; fi
	if test -e "/tmp/blkid.txt"; then rm "/tmp/blkid.txt"; fi
	if test -e "/tmp/lsblk.txt"; then rm "/tmp/lsblk.txt"; fi
	if test -e "/tmp/lsscsi.txt"; then rm "/tmp/lsscsi.txt"; fi
	if test -e "case_email_snippet.html"; then rm "case_email_snippet.html"; fi
	if test -e "case_render.html"; then rm "case_render.html"; fi
	if test -e "Disk_layout.html"; then rm "Disk_layout.html"; fi
	if test -e "case_email_outlook_snippet.html"; then rm "case_email_outlook_snippet.html"; fi
	...
	if test -e "/tmp/drive_selftest_tracking.csv"; then rm "/tmp/drive_selftest_tracking.csv"; fi
...
get_zpool_devices() {
    ...
    return_value="$(
        ...
        awk '
            ...
            # Any top-level vdev class
            start && /^[ \t]+(raidz|mirror|draid|spares|cache|logs|special)/ {
                invdev=1
                next
            }

            # Actual devices under the vdev
            start && invdev && /^[ \t]+[A-Za-z0-9]/ {
                print $1
            }
...
if [[ $Debug_Steps == "true" ]]; then echo "create_Email" | tee -a /tmp/multi_report_debug.txt; fi
	...
	if test -e "${logfile_header}"; then rm "${logfile_header}"; fi
		...
		if [[ $Monitor == "false" ]]; then
			...
			if [[ $UpdateRenderCaseAvailable == "true" || $UpdateRenderCaseAvailable == "update" ]] && [[ $Disklayout == "enable" && $softver == "Linux" ]]; then echo "<br>RENDER_CASE.PY UPDATE "$RendeerCase_Updated" --> V"$GitRenderCaseVersion" (Disk Layout)"; fi
...
#	cat $logfile > page0.html
#	cat $logfile_header > page00.html
	### Send report
...
# These settings will attempt to resend a failed message 5 times, once every 2 minutes,
#   then after 10 minutes it will resend once every 5 minutes for up to an hour.
sendemail_retry_count=11
sendemail_retry_delay=120
#cat $logfile_header > page000.html
#echo "Email to:"$Email
		if [[ $dump_all -ne 0 ]] || [[ $debug_sendemail == "debug_sendemail" ]]; then

			for ((sendemail_retry_loop=1; sendemail_retry_loop<30; sendemail_retry_loop++)); do	# are the retrys below the max count?
				...
				if [[ $? -eq 0 ]] || [[ sendemail_retry_loop -gt sendemail_retry_count ]]; then	break; fi
				if [[ sendemail_retry_loop -eq 6 ]]; then sendemail_retry_delay=300; fi				
				echo "Send Failed"
				echo "Retrying in $sendemail_retry_delay seconds..."
				sleep $sendemail_retry_delay
				echo "Retry Count: "$sendemail_retry_loop
			done

		else

			for ((sendemail_retry_loop=1; sendemail_retry_loop<30; sendemail_retry_loop++)); do
		...
		if [[ sendemail_retry_loop -eq sendemail_retry_count ]]; then
				echo " "
...
if [[ $Debug_Steps == "true" ]]; then echo "dump_drive_data $drive" | tee -a /tmp/multi_report_debug.txt; fi
	if [[ "$drive" != "cd0" ]]; then
		...
		if [[ $Partition_Backup == "enable" ]] && [[ $Attach_Files1 == "true" ]] && [[ $zip_partition_files != "true" ]]; then
			if test -e "/tmp/${tempfilepath}_${drive}_${serial1}.partition"; then
				if [[ "$truenas_sendmail_support" == "No" ]]; then
					(echo "create_attachment_file /tmp/${tempfilepath}_${drive}_${serial1}.partition ${serial1}_SGDISK_Partition.par" >> /tmp/attachment_files.txt)
				else
...
#		if [[ "$(echo "$statusOutput" | grep "done, " | grep "days")" ]]; then		# TRUE IF 'DAYS' PRESENT
			...
			if [ "$(echo "$statusOutput" | grep -w "scan" | awk '{print $4}')" = "progress" ]; then
				...
				currentTS="$(date "+%s")"
				scrubAge=$((((currentTS - scrubTS) + 43200) / 86400))
#				if [[ "$(echo "$statusOutput" | grep "scan" | awk '{print $6}')" == "with" ]]; then
#					# Yet another odd NVMe zpool oddity
...
#				fi
			...
			if [ "$(echo "$statusOutput" | grep -w "scan" | awk '{print $4}')" = "progress" ]; then
				...
				scrubAge="Never Scrubbed"
				scrubRepBytes="$Non_Exist_Value"
				scrubRepBytesfull="$Non_Exist_Value"
			fi
		fi
...
Alt_GPTID="disable"		# Disable this change for now, until testing has been conducted.
			###### FREEBSD VERSION OF THIS SECTION
			if [ $softver != "Linux" ]; then
			
				if [[ $Alt_GPTID == "enable" ]]; then
					#drives_in_gptid=$(zpool status "$pool" | grep "gptid" | awk '{print $1}')  # HDD and SSD

	# BELOW NEW WAY TO GET DRIVE GPTID VALUES.
					get_zpool_devices $pool		# returns list of drives in a pool
					drives_in_gptid=$return_value
				#	echo "gptid="$drives_in_gptid >> "core_alt_${pool}.txt"
					zpool status "$pool" | grep "gptid" | awk '{print $1}' >> "core_norm_${pool}"
				else
	# THE WAY WE HAVE BEEN DOING IT FOR YEARS
				...
				zpool_tdr_total=0
				zpool_tdw_total=0
#echo "gptid="$drives_in_gptid >> "scale_norm_${pool}.txt"
#get_zpool_devices $pool		# returns list of drives in a pool
#drives_in_gptid=$return_value
#echo "gptid="$drives_in_gptid >> "scale_alt_${pool}.txt"
...
#echo "ssdtdr="$ssdtdr" before math"
		ssdtdr="$((( $ssdtdr * 1073741824 ) / 1000000 ))"
#echo "ssdtdr="$ssdtdr
#echo "TRGiB="$ssdtdr
...
# Get Total Write GiB (Total_Writes_GiB)
		...
		ssdtdw="$((( $ssdtdw * 1073741824 ) / 1000000 ))"
...
if [[ $Debug_Steps == "true" ]]; then echo "get_smartHDD_listings" | tee -a /tmp/multi_report_debug.txt; fi
	if [ $softver != "Linux" ]; then
		...
		smartdrives=$(for drive in $(fdisk -l 2>/dev/null | grep "Disk /dev/sd" | cut -c 11-15 | cut -d ":" -f1 | tr '\n' ' '); do
			if [ "$(smartctl -i /dev/"${drive}" | grep "SMART support is:.\s*Enabled")" ] && ! [ "$(smartctl -i /dev/"${drive}" | grep "Solid State Device")" ]; then process_ignore_drives; fi
		done | awk '{for (i=NF; i!=0 ; i--) print $i }' | tr ' ' '\n' | sort | tr '\n' ' ')
	fi > /dev/null 2<&1
...
if [[ $Debug_Steps == "true" ]]; then echo "get_smartSSD_listings" | tee -a /tmp/multi_report_debug.txt; fi
	if [ $softver != "Linux" ]; then
		...
		smartdrivesSSD=$(for drive in $(fdisk -l 2>/dev/null | grep "Disk /dev/sd" | cut -c 11-15 | cut -d ":" -f1 | tr '\n' ' '); do
			if [ "$(smartctl -i /dev/"${drive}" | grep "SMART support is:.\s*Enabled")" ] && [ "$(smartctl -i /dev/"${drive}" | grep "Solid State Device")" ]; then process_ignore_drives; fi
		done | awk '{for (i=NF; i!=0 ; i--) print $i }' | tr ' ' '\n' | sort | tr '\n' ' ')
	fi > /dev/null 2<&1
...
if [[ $Debug_Steps == "true" ]]; then echo "get_smartNVM_listings" | tee -a /tmp/multi_report_debug.txt; fi
	if [ $softver != "Linux" ]; then
		...
		smartdrivesNVM=$(fdisk -l 2>/dev/null | grep "Disk /dev/nvm" | cut -d ':' -f 1 | cut -d '/' -f 3 | tr '\n' ' ')
		for drive in $smartdrivesNVM; do
			if [ "$(smartctl -i /dev/"${drive}" | grep "NVM")" ]; then process_ignore_drives; fi
		done | awk '{for (i=NF; i!=0 ; i--) print $i }' | tr ' ' '\n' | sort | tr '\n' ' '
			### Convert nvdx to nvmexx in smartdrivesNVM ###
...
if [[ $Debug_Steps == "true" ]]; then echo "get_smartOther_listings" | tee -a /tmp/multi_report_debug.txt; fi
	...
	if [ $softver != "Linux" ]; then
		...
		nonsmartdrives=$(for drive in $(fdisk -l 2>/dev/null | grep "Disk /dev/sd" | cut -c 11-13 | tr '\n' ' '); do
...
## Changed below line 2025_11_09 to "off" as it should have been
		if [[ "$Multipath" == "off" ]]; then
...
#echo "wearlevel 5:"$wearLevel"  Thresh:"$wearLevelThreshold"   Drive:"$drive
	elif [[ "$(echo "${smartdata5}" | jq -Mre '.ata_smart_attributes.table[]? | select(.name == "Perc_Avail_Resrvd_Space") | .value | values')" ]]; then
		wearLevel="$(echo "${smartdata5}" | jq -Mre '.ata_smart_attributes.table[]? | select(.name == "Perc_Avail_Resrvd_Space") | .value | values')"
		wearLevelThreshold="$(echo "${smartdata5}" | jq -Mre '.ata_smart_attributes.table[]? | select(.name == "Perc_Avail_Resrvd_Space") | .thresh | values')"
...
#echo "wearlevel 9:"$wearLevel"  Thresh:"$wearLevelThreshold"   Drive:"$drive
...
# Get Total Write GiB (Total_Writes_GiB)
		...
		ssdtdw="$((( $tdw * 1073741824 ) / 1000000 ))"  # Convert to bytes, now MB
...
## Made changes here 2025_11_07
	if [[ $lastTestType == "" ]]; then
		if [[ "$(echo "${smartdata5}" | grep "# 1" | awk '{print $4" "$5}' )" == *"short"* || "$(echo "${smartdata5}" | grep "# 1" | awk '{print $4" "$5}' )" == *"long"* ]]; then
			lastTestType="$(echo "${smartdata5}" | grep "# 1" | awk '{print $4" "$5}' )"
		fi
	fi


	if [[ $lastTestHours -eq 0 || $lastTestType == "" ]]; then
##
...
if [[ "$Monitor" == "true" ]]; then return; fi
# Set temp and color for Disk Layout
if [[ "$Disklayout" == "enable" ]] && [[ $softver == "Linux" ]]; then
	disk_layout_set_values
fi
	(
...
#echo "lastTestStatus:"$lastTestStatus > lastteststatus.txt
		if [[ $lastTestTypeColor == $bgColor ]] && [[ "$(echo $lastTestStatus | grep -i "progress" )" || "$(echo $lastTestType | grep -i "progress" )" ]]; then lastTestTypeColor=$blueColor; fi
		if [[ $Last_Test_Type_poh == "enable" ]] && [[ $Last_Test_Type_poh2 == "disable" ]]; then
			lastTestTypeColor=$yellowColor
			lastTestType=$lastTestType" (*)"
		fi
		
		if [[ $lastTestTypeHoursIdent != "yd" ]]; then
			...
			if [[ "$1" == "HDD" ]] && [[ "$HDD_Last_Test_Type" == "true" ]] && [[ "$Last_Test_Type_poh2" != "enable" ]]; then printf "<td style=\"text-align:center; background-color:%s; height:25px; border:1px solid black; border-collapse:collapse;\">%s</td>\n" "$lastTestTypeColor" "$lastTestType"; fi
			if [[ "$1" == "HDD" ]] && [[ "$HDD_Last_Test_Type" == "true" ]] && [[ "$Last_Test_Type_poh2" == "enable" ]]; then printf "<td style=\"text-align:center; background-color:%s; height:25px; border:1px solid black; border-collapse:collapse;\">%s</td>\n" "$lastTestTypeColor" "$lastTestType (${yrs}y ${dys_only}d)"; fi
			if [[ "$1" == "SSD" ]] && [[ "$SSD_Last_Test_Type" == "true" ]] && [[ "$Last_Test_Type_poh2" != "enable" ]]; then printf "<td style=\"text-align:center; background-color:%s; height:25px; border:1px solid black; border-collapse:collapse;\">%s</td>\n" "$lastTestTypeColor" "$lastTestType"; fi
			if [[ "$1" == "SSD" ]] && [[ "$SSD_Last_Test_Type" == "true" ]] && [[ "$Last_Test_Type_poh2" == "enable" ]]; then printf "<td style=\"text-align:center; background-color:%s; height:25px; border:1px solid black; border-collapse:collapse;\">%s</td>\n" "$lastTestTypeColor" "$lastTestType (${yrs}y ${dys_only}d)"; fi
			if [[ "$1" == "NVM" ]] && [[ "$NVM_Last_Test_Type" == "true" ]] && [[ "$Last_Test_Type_poh2" != "enable" ]]; then printf "<td style=\"text-align:center; background-color:%s; height:25px; border:1px solid black; border-collapse:collapse;\">%s</td>\n" "$lastTestTypeColor" "$lastTestType"; fi
			if [[ "$1" == "NVM" ]] && [[ "$NVM_Last_Test_Type" == "true" ]] && [[ "$Last_Test_Type_poh2" == "enable" ]]; then printf "<td style=\"text-align:center; background-color:%s; height:25px; border:1px solid black; border-collapse:collapse;\">%s</td>\n" "$lastTestTypeColor" "$lastTestType (${yrs}y ${dys_only}d)"; fi
...
#cat "$logfile" > page1_to_join.html
...
if [[ $Debug_Steps == "true" ]]; then echo "detailed_report" | tee -a /tmp/multi_report_debug.txt; fi
	...
	if [[ $Monitor == "false" ]]; then
		H_font_size=$(( font_size + 5 ))
	 	(
			echo "<pre style=\"font-size:$H_font_size; font-family: $font\">"
			 echo "<div style='font-weight:bold; font-size:22px;'><b>Multi-Report Text Section</b></div>"
#			echo "<b>Multi-Report Text Section</b>"
		...
		if [[ $Disklayout == "enable" ]] && [[ $softver == "Linux" ]]; then
			echo "4) Disk Layout - Beta (v"$VersionRenderCaseFilename")" >> "$logfile"
		else
			echo "4) Disk Layout - Beta (Disabled)" >> "$logfile"
		fi		
		echo " "  >> "$logfile"		
		...
		if [[ $SMR_Enable == "enable" ]]; then
			if echo "$logfile_warning" | grep -qi "SMR"; then echo "1) SMR Checking (v$VersionSMRFilename) - (Enabled) - WARNING" >> "$logfile"; else echo "1) SMR Checking (v$VersionSMRFilename) - (Enabled) - No Errors Detected" >> "$logfile"; fi
		else
...
while IFS= read -r line; do
	...
	if [[ "$(echo $src_file | grep -i ".html")" ]]; then new_directory="DISK_LAYOUT"; fi
	if [[ "$(echo $src_file | grep -i "disklayout_config.json")" ]]; then new_directory="DISK_LAYOUT"; fi
	if [[ "$(echo $src_file | grep -i "lsscsi.txt")" ]]; then new_directory="DISK_LAYOUT"; fi
	if [[ "$(echo $src_file | grep -i "blkid.txt")" ]]; then new_directory="DISK_LAYOUT"; fi
	if [[ "$(echo $src_file | grep -i "lsblk.txt")" ]]; then new_directory="DISK_LAYOUT"; fi
	if [[ "$(echo $src_file | grep -i "drive_selftest_tracking")" ]]; then new_directory="OTHER"; fi
...
#### CAN THE REST OF THIS FUNCTION BE MOVED INTO A SINGLE SECTION FOR ATTACHMENTS ?????   ######
	...
	if [[ "$dump_all" != "0" ]]; then
		...
		if [[ "$(echo $programver | grep -i "beta")" ]] || [[ $UpdateAvailable == "true" ]] || [[ $UpdateDriveAvailable == "true" ]] || [[ $UpdateSendemailAvailable == "true" ]]; then echo "<b><span style='color:darkred;'>"; fi
			...
			if [[ $UpdateRenderCaseAvailable == "true" ]] && [[ $Disklayout == "enable" ]] && [[ $softver == "Linux" ]]; then echo "<br>RENDER_CASE.PY (Disk Layout) UPDATE AVAILABLE --> "$GitSendemailVersion; fi
	...
	if [[ $Attach_Files1 == "true" ]]; then
		...
		if [[ $dump_all != "0" ]]; then
			...
			if test -e "$SCRIPT_DIR/drive_selftest_tracking.csv"; then
				cp "$SCRIPT_DIR/drive_selftest_tracking.csv" "/tmp/drive_selftest_tracking.csv"
				if [[ "$truenas_sendmail_support" == "No" ]]; then
					(echo "create_attachment_file /tmp/drive_selftest_tracking.csv drive_selftest_tracking.csv" >> /tmp/attachment_files.txt)
				else
					(
					echo "--${boundary}"
					echo "Content-Type: text/html"
					echo "Content-Transfer-Encoding: base64"
					echo "Content-Disposition: attachment; filename=drive_selftest_tracking.csv"
					base64 "/tmp/drive_selftest_tracking.csv"
					rm "/tmp/drive_selftest_tracking.csv"
					) >> "$logfile"
				fi
			fi
		...
		if test -e "/tmp/lsscsi.txt"; then
			if [[ "$truenas_sendmail_support" == "No" ]]; then
				(echo "create_attachment_file /tmp/lsscsi.txt lsscsi.txt" >> /tmp/attachment_files.txt)
			else
				(
				echo "--${boundary}"
				echo "Content-Type: text/html"
				echo "Content-Transfer-Encoding: base64"
				echo "Content-Disposition: attachment; filename=lsscsi.txt"
				base64 "/tmp/lsscsi.txt"
				rm "/tmp/lsscsi.txt"
				) >> "$logfile"
			fi
		fi		
		
		if test -e "/tmp/blkid.txt"; then
			if [[ "$truenas_sendmail_support" == "No" ]]; then
				(echo "create_attachment_file /tmp/blkid.txt blkid.txt" >> /tmp/attachment_files.txt)
			else
				(
				echo "--${boundary}"
				echo "Content-Type: text/html"
				echo "Content-Transfer-Encoding: base64"
				echo "Content-Disposition: attachment; filename=blkid.txt"
				base64 "/tmp/blkid.txt"
				rm "/tmp/blkid.txt"
				) >> "$logfile"
			fi
		fi
		
		if test -e "/tmp/lsblk.txt"; then
			if [[ "$truenas_sendmail_support" == "No" ]]; then
				(echo "create_attachment_file /tmp/lsblk.txt lsblk.txt" >> /tmp/attachment_files.txt)
			else
				(
				echo "--${boundary}"
				echo "Content-Type: text/html"
				echo "Content-Transfer-Encoding: base64"
				echo "Content-Disposition: attachment; filename=lsblk.txt"
				base64 "/tmp/lsblk.txt"
				rm "/tmp/lsblk.txt"
				) >> "$logfile"
			fi
		fi	

		if test -e "/tmp/disklayout_config.json"; then
			if [[ "$truenas_sendmail_support" == "No" ]]; then
				(echo "create_attachment_file /tmp/disklayout_config.json disklayout_config.json" >> /tmp/attachment_files.txt)
			else
				(
				echo "--${boundary}"
				echo "Content-Type: text/html"
				echo "Content-Transfer-Encoding: base64"
				echo "Content-Disposition: attachment; filename=disklayout_config.json"
				base64 "/tmp/disklayout_config.json"
				rm "/tmp/disklayout_config.json"
				) >> "$logfile"
			fi
		fi			
			...
			shopt -s nullglob
			files_part=(/tmp/*.partition)
			shopt -u nullglob
			if ((${#files_part[@]} > 0)); then
				zip_partition_files="true"	
				tar -cvf /tmp/drive_partition.tar /tmp/*.partition > /dev/null 2<&1
			fi
			
			if test -e "/tmp/drive_partition.tar"; then
				if [[ "$truenas_sendmail_support" == "No" ]]; then
					(echo "create_attachment_file /tmp/drive_partition.tar drive_partition.tar" >> /tmp/attachment_files.txt)
				else
					(
					echo "--${boundary}"
					echo "Content-Type: application/zip"
					echo "Content-Transfer-Encoding: base64"
					echo "Content-Disposition: attachment; filename=drive_partition.tar"
					base64 "/tmp/drive_partition.tar"
					sleep 1
					rm /tmp/drive_partition.tar
					) >> "$logfile"
				fi
			fi
...
##### THE NEXT LINE IS NOT LIKED BY 25.04 BUILD 14 JANUARY 2025
	...
	if [[ "$dump_all" != "0" ]]; then
		...
		for drive in $smartdrives; do
			echo "dump smartdrives" | tee -a /tmp/multi_report_debug.txt
			dump_drive_data
		done
		for drive in $smartdrivesSSD; do
			echo "dump smartdrivesSSD" | tee -a /tmp/multi_report_debug.txt
			dump_drive_data
		done
		for drive in $smartdrivesNVM; do
			echo "dump smartdrivesNVM" | tee -a /tmp/multi_report_debug.txt
			dump_drive_data
		done
		for drive in $nonsmartdrives; do
			echo "dump nonsmartdrives" | tee -a /tmp/multi_report_debug.txt
			dump_drive_data
		done
	...
	#	done
		...
		# Now tar the partition files together
...
if [[ $Debug_Steps == "true" ]]; then echo "crunch_numbers: $1" | tee -a /tmp/multi_report_debug.txt; fi
	...
	### Order of data -- $serial":"$tempwarn":"$tempcrit":"$sectorswarn":"$sectorscrit":"$reallocevent":"$multizonewarn":"$multizonecrit":"$rawreadwarn":"$rawreadcrit":"$seekerrorswarn":"$seekerrorscrit":"$testage":"$testAgeOvrd":"$heliummin":"$wearleveladj:farmoffset:testTypeOvrd"
	...
	if [[ $onHours != 0 ]]; then
		...
		if [[ $PowerTimeFormat == "ymdh" ]]; then onTime="${yrs}y ${mos}m ${dys}d ${hrs}h";
			...
			elif [[ $PowerTimeFormat == "yd" ]]; then onTime="${yrs}y ${dys_only}d";
			elif [[ $PowerTimeFormat == "y" ]]; then onTime="${yrs}y";
			elif [[ $PowerTimeFormat == "h" ]]; then onTime=$onHours;
			else onTime=$onHours;
		fi
		let ltyrs=$((($onHours / 8760)))
		let ltmos=$(((($onHours % 8760) / 730)))
		let ltdys=$((((($onHours % 8760) % 730) / 24)))
		let lthrs=$(((($onHours % 8760) % 730) % 24))
		let ltdys_only=$(((($onHours % 8760) / 24)))	
...
#echo "drive="$drive", smartStatus="$smartStatus
	...
	if [[ $reAllocEvent != "" ]]; then if [[ $(($reAllocEvent + 0)) -gt $SectorsCritx ]]; then reAllocEventColor=$critColor; else if [[ $(($reAllocEvent + 0)) -gt $ReAllocWarnx ]]; then reAllocEventColor=$warnColor; fi; fi; fi
	...
	else if [[ $(($reAllocEvent + 0)) -gt $ReAllocWarnx ]]; then logfile_warning=$logfile_warning"$(printf "Drive "$serial" Warning Reallocated Sectors Events "$reAllocEvent" - Threshold = "$ReAllocWarnx"<br>")"; fi; fi; fi
	...
	if [[ $offlineUnc != "" ]]; then
		if [[ $(($offlineUnc + 0)) -gt $SectorsCritx ]]; then
			offlineUncColor=$critColor
			logfile_critical=$logfile_critical"$(printf "Drive "$serial" Uncorrectable Sector Errors "$offlineUnc"<br>")"
		elif [[ $(($offlineUnc + 0)) -gt $SectorsWarnx ]]; then
			offlineUncColor=$warnColor
			logfile_warning=$logfile_warning"$(printf "Drive "$serial" Uncorrectable Sector Errors "$offlineUnc" - Threshold = "$SectorsWarnx"<br>")"
		fi
	else
		offlineUnc="$Non_Exist_Value"
	fi
	
#	if [[ $offlineUnc != "" ]]; then if [[ $(($offlineUnc + 0)) -gt $SectorsCritx ]]; then logfile_critical=$logfile_critical"$(printf "Drive "$serial" Uncorrectable Sector Errors "$offlineUnc"<br>")";
...
#	if [[ $offlineUnc == "" ]]; then offlineUnc="$Non_Exist_Value"; fi
	...
	if [[ $IgnoreUDMA != "enable" ]]; then if [[ $crcErrors != "$Non_Exist_Value" ]]; then if [[ $crcErrors != "0" ]]; then logfile_critical=$logfile_critical"$(printf "Drive "$serial" CRC Errors "$crcErrors"<br>")"; deviceStatusColor=$critColor; fi; fi; fi
...
#		if [[ "$TempDisplay" == "&deg; C" ]]; then TempDisplay="° C"; fi
		...
		echo '### Enter your Email address to send the report to.  To send to multiple email addresses separate those using a comma ",".'
		...
		echo 'External_Script_Name="'$External_Script_Name'"	# Default setting is "$SCRIPT_DIR/drive_selftest.sh"'
		...
		echo 'Check_For_Updates_Day_of_Week="'$Check_For_Updates_Day_of_Week'"		# Will only check for an update on this day of the week.  Default = "Mon" ("Tue","Wed","Thu","Fri","Sat","Sun","All","Month")'
		...
		echo 'Automatic_Sendemail_Update="'$Automatic_Sendemail_Update'"	# WARNING !!!  This option will automatically update the Sendemail.py script if a newer version exists on GitHub with no user interaction. Default = "disable"'
		...
		echo " "
		echo "###### Disk Layout ######"
		echo 'Disklayout="'$Disklayout'"			# SCALE ONLY.'
		echo 'High_Contrast="'$High_Contrast'"		# This will produce a high contrast output.  Default="false".'
		echo " "
		...
		echo 'PowerTimeFormat="'$PowerTimeFormat'"			# Format for power-on hours string, valid options are "ymdh", "ymd", "ym", "yd", "y", or "h" (year month day hour).'
		...
		echo 'HDD_Total_Data_Written_30day_Title="'$HDD_Total_Data_Written_30day_Title'"'
		echo " "
		...
		echo 'SSD_Total_Data_Written_30day_Title="'$SSD_Total_Data_Written_30day_Title'"'
		echo " "
		...
		echo 'NVM_Total_Data_Written_30day_Title="'$NVM_Total_Data_Written_30day_Title'"'
		echo " "
		...
		echo '# rawreadcrit:seekerrorswarn:seekerrorscrit:testage:testAgeOvrd:heliummin:wearleveladj:farmoffset:testTypeOvrd'
		echo '# (testAgeOvrd/testTypeOvrd '0'=Default, '1'=Ignore, wearleveladj 'd'=Default 'r'=Reverse value 'n'=Normalized 'i'=Ignore, farmoffset 'd' or 'i')'
		echo " "
        ...
        echo '              	                	    #  A value of 0 (zero) will disable all SMART test(s) on the affected pool during a SCRUB operation.  Default=60'
        echo '          	                     	    #  NOTE: A RESILVER operation on a pool automatically cancels SMART Long testing for that pool to put priority on rebuilding the pool.'
...
if [[ $Debug_Steps == "true" ]]; then echo "generate_config_file" | tee -a /tmp/multi_report_debug.txt; fi
	...
	for (( z=1; z<=50; z++ )); do
		...
		echo "      L)ayout for drives (AKA Disk Layout)"
...
#							shopt -s nocasematch
						...
						G)	# QUESTIONS UPDATED
							...
							echo -n 'valid options are "ymdh", "ymd", "ym", "yd", "y", or "h" (year month day hour): '
							read Keyboard_yn
						...
						K)	# QUESTIONS UPDATED
							...
							echo "-- Automatic Compensation"
							echo "-- Manual Compensation Entries"
							echo "-- Skip the compensation questions"
							echo "Enter 'a' for automatic, 'm' for manual, or Return to skip this section."
							read -s -n 1 Keyboard_yn
							...
							if [[ ! $autoselect == "1" ]] && [[ $Keyboard_yn == "m" ]]; then
								echo " "
								echo " "
...
#								farmoffset="d"
				...
				L)
					...
					echo "            Disk Layout"
					echo " "
					echo "   C)ontrast Layout Output (enable/disable)"
					echo "   D)isk Layout (enable/disable)"
					echo " "
					if [[ $High_Contrast == "true" ]]; then High_Contrast_altered="enable"; else High_Contrast_altered="disable"; fi
					echo "     High Contrast Current Value: "$High_Contrast_altered
					echo "     Disk Layout Feature Current Value: "$Disklayout
					echo " "
					echo " "
					echo "   H)elp"
					echo "   W)rite to configuration file  (NOTE: Any changes will be lost if you do not write the file NOW)"
					...
					case $Keyboard_loop2 in
						...
						C)
							clear
							echo "            Disk Layout - High Contrast"
							echo " "
							echo "This setting will Enable or Disable the High Contrast Output."
							echo " "
							echo " "
							echo " "
							echo "-- Do you want to enable or disable the High Contrast option?"
							echo " "
							# Need to update case_render.py to accept true/false and enable/disable
						#	if [[ $High_Contrast == "true" ]]; then High_Contrast_altered="enable"; else High_Contrast_altered="disable"; fi
							echo "Current Value: "$High_Contrast_altered
							echo " "
							echo "(e or enable, or d or disable, or Return to skip)"
							read Keyboard_yn
							echo " "
							if [[ $Keyboard_yn != "" ]]; then
								if [[ $Keyboard_yn == "e" ]] || [[ $Keyboard_yn == "enable" ]]; then High_Contrast="true"; else High_Contrast="false"; fi
							fi
							echo " "
							if [[ $High_Contrast == "true" ]]; then High_Contrast_altered="enable"; else High_Contrast_altered="disable"; fi
							echo "New Value: "$High_Contrast_altered
							echo " "
							echo " "
							echo -n "Press Any Key to Continue"
							read -s -n 1 Keyboard_var3
						...
						D)
							clear
							echo "            Disk Layout - Feature"
							echo " "
							echo "This setting will Enable or Disable the Disk Layout Feature."
							echo " "
							echo " "
							echo " "
							echo "-- Do you want to enable or disable the Disk layout Feature?"
							echo " "
							echo "Current Value: "$Disklayout
							echo " "
							echo "(e or enable, or d or disable, or Return to skip)"
							read Keyboard_yn
							echo " "
							if [[ $Keyboard_yn != "" ]]; then
								if [[ $Keyboard_yn == "e" ]] || [[ $Keyboard_yn == "enable" ]]; then Disklayout="enable"; else Disklayout="disable"; fi
							fi
							echo " "
							echo "New Value: "$Disklayout
							echo " "
							echo " "
							echo -n "Press Any Key to Continue"
							read -s -n 1 Keyboard_var3
						...
						H)
							clear
							echo "            Disk Layout - Help"
							echo " "
							echo "Disk Layout is a feature which allows the user to generate a graphic map"
							echo "which will help identify the physical location of a drive."
							echo " "
							echo "After enabling the feature, then runthe script one time.  You will be presented"
							echo "with a URL in which you will be able to configure your Disk Layout map."
							echo " "
							echo "Each time you run Multi-Report, the Disk Layout chart will be inserted between"
							echo "the bottom of the original charts and begining of the Text Section."
							echo " "
							echo " "
							echo "Press any key to continue"
							read -s -n 1 key							
						...
						*)
							echo "You have brains in your head. You have feet in your shoes. You can steer yourself any direction you choose. You're on your own. And you know what you know. And YOU are the one who'll decide where to go..."
							sleep 5
						;;
...
if [[ $Debug_Steps == "true" ]]; then echo "display_help" | tee -a /tmp/multi_report_debug.txt; fi
	...
	echo "                       and then exit."
	echo " "
	echo "      -debug_sendemail  This will enable debug mode and a file will be written to /sendemail_log"
	echo " "
	echo "Page 2 of 12"
	...
	read -s -n 1 Keyboard_yn
	echo " "
	echo "      -check_for_updates   Manually check for any updates.  This results in"
	echo "                           a listing of current available versions."
	echo " "
	echo "      -update       	Manually check/update GitHub for a Multi-Report update."
	echo " "
	echo "      -update_selftest	Manually check/update GitHub for a Drive_Selftest update."
	echo " "
	echo "      -update_sendemail	Manually check/update GitHub for a Sendemail update."
	echo " "
	echo "      -update_render	Manually check/update GitHib for a Render_Case (Disk Layout) update."
	echo " "
	echo "      -update_all   Manually check/update GitHub for ALL files for an update."
	echo " "
...
if [[ "$1" == "-dump" || "$3" == "-dump" ]]; then
	...
	# -dump config = dump Multi-Report config, Truenas Config, and statistical data to owner.
...
if ! [[ "$1" == "-config" || "$1" == "-help" || "$1" == "-delete" || "$1" == "-s" || "$1" == "" || "$1" == "-dump" || "$1" == "-purge" || "$1" == "-h" || "$1" == "-t" || "$1" == "-m" || "$1" == "-u7zip" || "$1" == "-update" || "$1" == "-update_selftest" || "$1" == "-update_sendemail" || "$1" == "-update_smr" || "$1" == "-update_all" || "$1" == "-ignore_lock" || "$1" == "-cleanup" || "$1" == "-scsismart" || "$1" == "-update_smr" || "$1" == "-check_smr"  || "$1" == "-disable_smr" || "$1" == "-enable_smr" || "$1" == "-disable_smr_alarm" || "$1" == "-enable_smr_alarm" || "$1" == "-disable_farm" || "$1" == "-enable_farm" || "$1" == "-tardrivedata"  || "$1" == "-test_sendemail" || "$1" == "-check_for_updates" || "$1" == "-update_render" || "-dev" || "-debug_sendemail" ]]; then
...
if [[ "$1" == "-update_render" ]]; then
	update_RenderCase_script
	echo "The Render_Case (Disk Layout) python script has been updated.  Exiting." | tee -a /tmp/multi_report_debug.txt
	sleep 1
	exit 0
fi
...
if [[ $Total_Data_Written_Month == "month" ]]; then
	...
	HDD_Total_Data_Written_Month_Title=$HDD_Total_Data_Written_30day_Title
	SSD_Total_Data_Written_Month_Title=$SSD_Total_Data_Written_30day_Title
	NVM_Total_Data_Written_Month_Title=$NVM_Total_Data_Written_30day_Title
fi
...
if [[ $Check_For_Updates == "enable" ]] && [[ "$1" != "-t" && "$1" != "-dump" ]]; then
	Now=$(date +"%a")
	case $Check_For_Updates_Day_of_Week in
		All)
			run_check="true"
		;;

		Mon|Tue|Wed|Thu|Fri|Sat|Sun|All)
			if [[ "$Check_For_Updates_Day_of_Week" == "$Now" ]]; then run_check="true"; fi
		;;

		Month)
			if [[ $(date +"%d") == "01" ]]; then run_check="true"; fi
		;;
...
if [[ "$UpdateRenderCaseAvailable" == "true" ]] && [[ $Disklayout == "enable" ]] && [[ $softver == "Linux" ]]; then
	logfile_update=$logfile_update"- Render_Case.py (Disk Layout) Update Is Available. Use '-update_render' switch to update.<br>"
fi
...
if [[ "$1" == "-debug_sendemail" || "$2" == "-debug_sendemail" || "$3" == "-debug_sendemail" ]]; then
	debug_sendemail="debug"
	echo "Sendemail.py Debug Set --> Look in /sendemail_log directory for debug log."
fi

if [[ "$1" == "-dev" ]]; then
	echo "In Developer Mode - SMART Tests will not be run."
	External_SMART_Testing="disable"
	echo "Enabling Debug_Steps --> Shows which functions are being executed, along with additional data for troubleshooting."
	Debug_Steps="true"
fi
...
# >>>>>>>>>>>>>>>>>>>>>>>> INSERT MAPPER SCRIPT PART 1 HERE <<<<<<<<<<<<<<<<<<<<<<<<<
if [[ "$Disklayout" == "enable" ]] && [[ $softver == "Linux" ]]; then
	# Do we have the file `render_case.py` ?
	if ! test -e "${SCRIPT_DIR}/render_case.py"; then
		if [[ "$(curl -is https://github.com | head -n 1)" ]]; then
...
# Before we go and grab individual drive data, we need to initialize the Mapper Arrays

	# Read the array if it exists, this preserves the slot and cage configuration
	if test -e "$disk_layout_json_file"; then
#		echo $disk_layout_json_file" exists, reading the file...." | tee -a debug.txt
		disk_layout_read_json_file
	fi
	
	# Collect the drive data regardless of if the disklayout_config.json file exists or not.
	# At a minimum this will gather data for a new file.
		disk_layout_collect_drive_data
		disk_layout_reconcile_data
fi
# <<<<<<<<<<<<<<<<<<<<<<< END MAPPER SCRIPT PART 1 >>>>>>>>>>>>>>>>>>>>>>>>
...
# >>>>>>>>>>>>>>>>>>>>>> MAPPER SCRIPT PART 2 <<<<<<<<<<<<<<<<<<<<

if [[ "$Disklayout" == "enable" ]] && [[ $softver == "Linux" ]]; then
	# Write the arrays to the disklayout_config.json file.  This will create a new file if one didn't previously exist.
	disk_layout_write_json_file
	
	# Remove any duplicates and clear those slot numbers.
#	disk_layout_remove_duplicates
	
	# Generate the HTML file
	python3 render_case.py --config "$disk_layout_json_file" > /dev/null 2<&1
	exit_code=$?
	if [[ "$exit_code" != 0 ]]; then
		echo "<><><><><><><><><><><><><><><><><><><><><>"
		echo "<>          Disk Layout - Error         <>"
		echo "<>   render_case had an error code = $exit_code  <>"
		echo "<><><><><><><><><><><><><><><><><><><><><>"
	fi
	# Add HTML to report
	disk_layout_join_html
