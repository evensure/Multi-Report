...
#  3. Incorporate HTML into output, just attach to the end of the CHART section, before the Text Section.
...
#  DONE - 7. Add Drive Mapper status (enabled/disabled) to the Text Report
...
#  DONE - 15. Fixed iniial json file creation to have `"case": null`.
#  DONE - 16. Changed configuraiton file name from "config.json" to "disklayout_config.json".
...
Automatic_RenderCase_Update="disable"	# This is for the Drive Mapper
...
###### Disk Layout ######
Disklayout="disable"		# BETA TESTING ONLY  Options: "enable" or "disable".
...
CurrentFilename="multi_report_v3.26_2025_11_04.txt"
...
#  echo "<title>ZFS + NVMe Combined Report</title>"
  echo "<style>"
  # Optional: unify background & fonts and ensure pre-blocks are readable
...
# Use awk to split at the second </table>
awk -v out1="$part1" -v out2="$part2" '
{
  ...
  if (count == 2) {
    close(out1)
    # Start writing to part2 from the *next* line
...
#	echo "Start -----------------------------------------------------------------------" | tee -a debug.txt
#	echo "lsscsi_line="$lsscsi_line | tee -a debug.txt
...
#		echo "Bypass - Not a Disk" | tee -a debug.txt
        ...
        continue
#	else
#		echo "MAPPER_DEVICE_NAME="$MAPPER_DEVICE_NAME | tee -a debug.txt
    fi
...
#	echo "MAPPER_MODEL_NAME="$MAPPER_MODEL_NAME | tee -a debug.txt  
...
#		echo "mapper_serial=${mapper_serial[array_index]}" | tee -a debug.txt
...
#        echo "mapper_capacity=${mapper_capacity[array_index]}" | tee -a debug.txt
...
#		if [[ ${mapper_uuid[array_index]} != "" ]]; then
#			echo "mapper_uuid=${mapper_uuid[array_index]}" | tee -a debug.txt
#		else
#			echo "mapper_uuid=${mapper_uuid[array_index]} is not assigned a UUID, may be a spare." | tee -a debug.txt
#		fi
		
        # Get drive address
        mapper_addr[array_index]=$(echo "$MAPPER_CAP_ADDR_LINE" | cut -d'[' -f2 | cut -d']' -f1)
#		echo "ADDRESS=${mapper_addr[array_index]}" | tee -a debug.txt
...
#		echo "mapper_pool=${mapper_pool[array_index]}" | tee -a debug.txt
        
#		echo "Drive Color=${mapper_color[array_index]}" | tee -a debug.txt
		
#		echo "Drive Temp=${mapper_temperature[array_index]}" | tee -a debug.txt
			
#		echo "mapper_slot="${mapper_slot[array_index]} | tee -a debug.txt
...
#echo "---" | tee -a debug.txt
#echo "Processed $(( array_index )) actual drives." | tee -a debug.txt

}

mapper_read_json_file() {
if [[ $Debug_Steps == "true" ]]; then echo "mapper_read_json_file"; fi
	# We need to read the json file and save the data locally.
	# 
#	echo "READ JSON FILE FUNCTION" | tee -a debug.txt
...
#	echo "$mapper_json_file file:  Version:$mapper_read_version, Date:$mapper_read_version_date" | tee -a debug.txt
...
#		echo "mapper_slot="${mapper_slot[${lsscsi_line}]} | tee -a debug.txt
#		echo "Pool:"${mapper_pool[${lsscsi_line}]} | tee -a debug.txt
#		echo "Drive ID:"${mapper_name[${lsscsi_line}]} | tee -a debug.txt
#		echo "Serial:"${mapper_serial[${lsscsi_line}]} | tee -a debug.txt
#		echo "Model:"${mapper_model[${lsscsi_line}]} | tee -a debug.txt
#		echo "Capacity:"${mapper_capacity[${lsscsi_line}]} | tee -a debug.txt
#		echo "UUID:"${mapper_uuid[${lsscsi_line}]} | tee -a debug.txt
#		echo "Drive Color:"${mapper_color[${lsscsi_line}]} | tee -a debug.txt
#		echo "Drive Temp="${mapper_temperature[${lsscsi_line}]} | tee -a debug.txt
...
EOF
   ...
   if test -e "$mapper_json_file"; then
		...
		echo "Creating Initial Drive Mapper $mapper_json_file File"
...
# Go git the file "render_case.py"
	...
	# Do we have the file `render_case.py` ?
		...
		if test -e "/tmp/Multi-Report/render_case.py"; then
			...
			echo -n "   Drive Mapper (render_case) - Installed: "$VersionRenderCaseFilename", GitHub: "$GitRenderCaseVersion
...
if [[ $Debug_Steps == "true" ]]; then echo "cleanup_files" | tee -a /tmp/multi_report_debug.txt; fi
	...
	if test -e "case_email_render.html"; then rm "case_render.html"; fi
...
# These settings will attampt to resend a failed message 5 times, once every 2 minutes,
...
if [[ "$Monitor" == "true" ]]; then return; fi
# Set temp and color for Drive Mapper
if [[ "$Disklayout" == "enable" ]]; then
	mapper_set_values
fi
	(
...
#		if [[ "$TempDisplay" == "&deg; C" ]]; then TempDisplay="° C"; fi
		...
		echo " "
		echo "###### Disk Layout ######"
		echo 'Disklayout="'$Disklayout'"			# BETA TESTING ONLY'
		echo " "
